...
**Test seam**: `grid.py`, `camera.py`, `day_night.py`, `coords.py`, `pathfinding.py`, `npc.py`, `inventory.py`, `task.py`, `gather_task.py`, `expand_task.py`, `build_task.py`, `combat.py`, `nest.py`, `monster.py`, `game_over.py`, `blocking.py`, `save.py`, `population.py` never `import pygame` themselves — unit tested directly with pytest, no display needed. `game.py` is the pygame-coupled integration shell (window, event loop, rendering, click handling) — not unit tested, verified via `tests/smoke_render.py` (headless, `SDL_VIDEODRIVER=dummy`) plus manual play. Caveat since ticket 04: `plugins.py` is the one deliberate exception — it imports `render_buildings` (pygame-coupled), so importing it (and transitively `world.py`, which imports `plugins` for its registration side effects) pulls pygame into `sys.modules`. Harmless for tests (no display/font calls happen at import time, confirmed by the full suite passing with no `SDL_VIDEODRIVER` set), but not literally zero-pygame — don't add real pygame calls to any module on this list. Keep new simulation modules on this pygame-call-free side of the line.

**Not yet built** (source doesn't exist for these): magic, taming, skills. Everything else in the core-loop tickets (buildings, combat, hunger/starvation, game over/scoring, priority table UI, HUD, save/load checkpoint) is implemented.
